...
public class KarmaPassiveEffect extends SpiritualitePassiveEffect {
    ...
    public void onEvent(GameEvent event) {
        ...
        if (event instanceof SpellCostAdjustEvent e) {
            if (e.getSpell().getSpiritualite() == null || this.getSpiritualite() == null
            ...
            if (personnage.getPassiveState("karma_locked", 0) == 1) {
                if (isRestorative) {
                    ...
                    if (lockedDuration <= 0) {
                        System.out.println(
                                "✨ Le Karma de " + personnage.getName() + " est rétabli par un sort d'Harmonie !");
                        ...
                        System.out.println("⏳ Le Karma de " + personnage.getName() + " s'apaise. Tours restants : "
                                + lockedDuration);
            ...
            if (gauge <= -4 || gauge >= 4) {
                if (gauge <= -4) {
                    System.out.println(
                            "💥 Le Karma est corrompu par les ténèbres. La voie du Karma est verrouillée pour 6 tours.");
                    ConsumableSpellBuffDebuffEffect darkBuff = new ConsumableSpellBuffDebuffEffect(1, 1.5);
                    personnage.addConsumableSpellBuff(darkBuff);
                } else {
                    System.out.println(
                            "💥 Le Karma est aveuglé par la lumière. La voie du Karma est verrouillée pour 6 tours.");
