...
import { GLOBAL_STAT_LABELS, GLOBAL_SRC_LABELS } from './utils/constants.js';
...
export function getSpellColor(sp) {
    if (sp.voie && sp.voie.nom) {
        ...
        if (vNom.includes('karma')) {
            if (sp.karmaAlignment === 'PROTECTIVE') return '#fde047'; // Discret vers jaune (Lumière)
            if (sp.karmaAlignment === 'OFFENSIVE') return '#d8b4fe'; // Discret vers violet (Ténèbres)
    ...
    if (sp.spiritualite && (sp.spiritualite.nom || typeof sp.spiritualite === 'string')) {
        ...
        if (sNom.includes('karma')) {
            if (sp.karmaAlignment === 'PROTECTIVE') return '#fde047';
            if (sp.karmaAlignment === 'OFFENSIVE') return '#d8b4fe';
            return '#e7d198';
        }
