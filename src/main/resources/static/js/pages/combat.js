...
async function fetchCombatEquipments(sessionId) {
    try {
                ...
                if (subType === 'ALTERATION') {
                    ...
                    if (data.currentRoom.lootTable && data.currentRoom.lootTable.length > 0) {
                        ...
                        data.currentRoom.lootTable.forEach((entry, idx) => {
                            ...
                            let specialItemNameArg = entry.priceSpecialItemName ? `'${entry.priceSpecialItemName.replace(/\\/g, "\\\\").replace(/'/g, "\\'").replace(/"/g, '&quot;').replace(/\n/g, ' ').replace(/\r/g, '')}'` : 'null';
                            let safeName = nameHtml.replace(/\\/g, "\\\\").replace(/'/g, "\\'").replace(/"/g, '&quot;').replace(/\n/g, ' ').replace(/\r/g, '');
                            let buttonHtml = '';
                            if (isPurchased) {
