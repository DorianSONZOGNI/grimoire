...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (!dungeonId || !characterIds) {
        alert("Param├¿tres de combat manquants.");
...
function initiateCombatCast(spellId) {
    ...
    if (spellId !== null && cardEl) {
        ...
        if (multiEnemy || multiAlly) {
            overlay.innerHTML = `
                <span style="font-size: 0.9rem; font-weight: 600; color: #e2e8f0;">S├®lectionnez une cible</span>
...
async function nextRoom() {
    ...
    try {
        ...
        console.error(e);
        showNotif("Erreur lors du passage ├á la salle suivante", true);
    }
}
...
async function acceptAlteration() {
    ...
    try {
        ...
        console.error(e);
        showNotif("Erreur lors de l'alt├®ration", true);
    }
}
...
async function buyMerchantItem(lootIndex) {
    ...
    try {
        ...
        if (!res.ok) {
            alert("Vous n'avez pas les ressources n├®cessaires, ou cet objet n'est plus disponible.");
...
function generateEquipmentTooltipHTML(eq) {
    ...
    const statsDef = [
        ...
        { key: 'bonusResistance', label: 'R├®s', icon: 'shield', color: '#10b981' },
...
async function openChest() {
    ...
    try {
        ...
        if (lootContainer) {
            ...
            // Also show gained items (check logs for "Vous avez trouv├® un objet")
            let gainedItemsHtml = '';
            let goldAmount = 0;
            if (data.combatLog) {
                let chestLogs = [];
                ...
                chestLogs.forEach(log => {
                    const itemNameMatch = log.match(/Vous avez trouv├® un objet : (.*) !/);
                    if (itemNameMatch) {
...
function updateUI(data) {
    ...
    document.getElementById('headerDungeonName').textContent = data.donjon.name + " - ├ëtape " + (data.currentRoomIndex + 1);
    ...
    if (data.currentRoom) {
        if (data.currentRoom.type === 'COMBAT') {
                ...
                if (vicOverlay) {
                    ...
                    if (xpContainer) {
                        ...
                        if (data.combatLog) {
                            for (let i = data.combatLog.length - 1; i >= 0; i--) {
                                const log = data.combatLog[i];
                                if (log.includes("Les monstres vaincus ont l├óch├®")) {
                                    const goldMatch = log.match(/l├óch├® (\d+) Or/);
                                    if (goldMatch) goldAmount = parseInt(goldMatch[1]);
                                    break;
                                }
            ...
            if (subType === 'ALTERATION') {
                ...
                title.textContent = 'Alt├®ration';
                desc.innerHTML = data.currentRoom.eventText || 'Une force myst├®rieuse vous entoure...';
                ...
                if (!data.roomEventCompleted && data.currentRoom.alterationType !== 'RIEN') {
                    ...
                    if (data.currentRoom.alterationType === 'VIE_XP') {
                        ...
                        warningHtml = `<div style="color: #ef4444; font-size: 0.85rem; margin-top: 0.5rem; text-align: center; background: rgba(239, 68, 68, 0.1); padding: 0.5rem; border-radius: 6px; border: 1px solid rgba(239, 68, 68, 0.3);"><span class="material-symbols-outlined" style="font-size: 1rem; vertical-align: middle;">warning</span> <strong>Attention :</strong> L'item "${data.currentRoom.alterationRequiredItem || 'sp├®cial'}" sera d├®finitivement d├®truit de l'inventaire d'un de vos h├®ros s'il accepte cette offre.</div>`;
                    }
                ...
                if (data.currentRoom.lootTable && data.currentRoom.lootTable.length > 0) {
                    ...
                    data.currentRoom.lootTable.forEach((entry, idx) => {
                        ...
                        if (entry.equipment) {
                            ...
                            tooltipDataHtml = `<div style="padding: 0.5rem; color: #d946ef; font-weight: 600; text-align: center; min-width: 150px;">Objet Sp├®cial</div><div style="color: #94a3b8; text-align: center; font-size: 0.9rem;">Cet objet aura un effet unique !</div>`;
                        }
                ...
                title.textContent = 'Pi├¿ge !';

                let trapDesc = data.currentRoom.eventText || 'Un pi├¿ge se d├®clenche !';

                if (data.roomEventCompleted) {
                    trapDesc += `<br><br><span style="color:#10b981;">­ƒ¬ó Pi├¿ge ├®vit├® gr├óce ├á une Corde !</span>`;
                    ...
                    if (trapDetails.length > 0) {
                        trapDesc += `<br><br>ÔÜá´©Å Perte de : ` + trapDetails.join(' et ');
                    }
                    ...
                    if (data.currentRoom.trapHasRopeOption) {
                        ...
                        lootContainer.innerHTML = `
                                <div style="display: flex; flex-direction: column; align-items: center; width: 100%;">
                                    <div style="display: flex; gap: 1rem; margin-top: 1rem; justify-content: center; width: 100%;">
                                        ...
                                        <button class="btn" style="flex: 1; max-width: 250px; background: rgba(255, 255, 255, 0.05); color: #94a3b8; border: 1px solid rgba(255, 255, 255, 0.1); padding: 0.8rem; border-radius: 8px; font-weight: 600; cursor: pointer; transition: all 0.2s ease;" onclick="nextRoom()">Subir le pi├¿ge et passer</button>
                                    </div>
                                </div>
                            `;
                        btnCont.style.display = 'none';
                        ...
                        btnCont.style.display = 'block';
                        btnCont.textContent = 'Subir le pi├¿ge et passer';
                        lootContainer.style.display = 'none';
                    }
                }
                ...
                title.textContent = 'Porte ├ëtrange';
                desc.innerHTML = data.currentRoom.eventText || 'Une porte myst├®rieuse se dresse devant vous...';
                    ...
                    if (outcomes.length > 0) {
                        ...
                        lootContainer.innerHTML = `
                                <div style="color: #94a3b8; font-size: 0.85rem; text-align: center; width: 100%;">
                                    <span style="color: #fbbf24; font-weight: 600;">Que se cache-t-il derri├¿re ?</span><br>
                                    Le r├®sultat sera r├®v├®l├® si vous passez la porte...
                                </div>
                            `;
                    }
                }
...
data.combatLog.forEach(log => {
    ...
    text = text.replace(/inflige (\d+) d├®g├óts/g, 'inflige <span style="color:#f59e0b;font-weight:bold;">$1</span> d├®g├óts');
...
function generateFighterHtml(c, isHero) {
    ...
    if (c.shieldTotal > 0) hpLabel += ` (+${c.shieldTotal} ­ƒøí´©Å)`;
    ...
    statsHtml += `<span class="hero-stat-chip"><span class="material-symbols-outlined" style="color: #10b981;">shield</span>${res} R├®s</span>`;
    ...
    if (c.voie && c.voie.nom && c.voie.nom.toLowerCase().includes('destruction')) {
        ...
        statsHtml += `<span class="hero-stat-chip" title="Chaleur accumul├®e" style="border-color: rgba(249, 115, 22, 0.4);"><span class="material-symbols-outlined" style="color: #f97316;">local_fire_department</span>${heat}/100</span>`;
    }

    if (c.voie && c.voie.nom && (c.voie.nom.toLowerCase().includes('surete') || c.voie.nom.toLowerCase().includes('s├╗ret├®'))) {
        let suretePoints = 0;
        if (c.passiveStates && c.passiveStates['surete_points'] !== undefined) {
            suretePoints = c.passiveStates['surete_points'];
        }
        statsHtml += `<span class="hero-stat-chip" title="Points de S├╗ret├®" style="border-color: rgba(20, 184, 166, 0.4);"><span class="material-symbols-outlined" style="color: #14b8a6;">security</span>${suretePoints}</span>`;
    }
    ...
    if (c.voie && c.voie.nom && c.voie.nom.toLowerCase().includes('trahison')) {
        ...
        statsHtml += `<span class="hero-stat-chip" title="1er attaque physique du tour (+10% d├®g├óts physiques)" style="${styleBase}"><span class="material-symbols-outlined" style="color: inherit;">bolt</span>+10%</span>`;
        statsHtml += `<span class="hero-stat-chip" title="Cible < 50% PV (+15% d├®g├óts physiques)" style="${styleLowHp}"><span class="material-symbols-outlined" style="color: inherit;">heart_broken</span>+15%</span>`;
        statsHtml += `<span class="hero-stat-chip" title="Cible avec D├®buff (+10% d├®g├óts physiques)" style="${styleDebuff}"><span class="material-symbols-outlined" style="color: inherit;">trending_down</span>+10%</span>`;
    }

    if (c.voie && c.voie.nom && (c.voie.nom.toLowerCase().includes('cr├®ation') || c.voie.nom.toLowerCase().includes('creation'))) {
    ...
    if (c.voie && c.voie.nom && c.voie.nom.toLowerCase().includes('consolidation')) {
        ...
        let icon = 'shield', color = '#9ca3af', borderColor = 'rgba(156, 163, 175, 0.4)', text = '+5% Armure', title = "Consolidation (D├®faut)";
        if (level === 1) {
            ...
            icon = 'security'; color = '#a855f7'; borderColor = 'rgba(168, 85, 247, 0.4)'; text = '+10% R├®sist'; title = "Consolidation (Niveau 3)";
        } else if (level === 4) {
            icon = 'water_drop'; color = '#3b82f6'; borderColor = 'rgba(59, 130, 246, 0.4)'; text = '-20% Co├╗t'; title = "Consolidation (Niveau 4)";
        } else if (level === 5) {
            icon = 'gpp_good'; color = '#eab308'; borderColor = 'rgba(234, 179, 8, 0.4)'; text = '+8% Arm/R├®s'; title = "Consolidation (Niveau 5)";
        }
    ...
    if (hasKarma) {
        ...
        if (karmaLocked) {
            ...
            text = 'Bris├®'; title = "Karma Bris├® (Voie d├®sactiv├®e)";
            ...
            text = `${karmaGauge}/3`; title = "Karma T├®n├¿bres";
        } else if (karmaGauge > 0) {
            borderColor = 'rgba(253, 224, 71, 0.4)'; color = '#fde047'; icon = 'light_mode';
            text = `+${karmaGauge}/3`; title = "Karma Lumi├¿re";
        } else {
    ...
    if (isEsprit) {
        ...
        const text = canCast ? '├ëveill├®' : 'Bloqu├®';
        const title = "Condition Esprit : >= 20% PV ET Mana";
        statsHtml += `<span class="hero-stat-chip" title="${title}" style="border-color: ${borderColor}; color: ${color};"><span class="material-symbols-outlined" style="color: inherit;">${icon}</span>${text}</span>`;
    }

    const isTenebres = c.spiritualite && c.spiritualite.nom && (c.spiritualite.nom.toLowerCase().includes('t├®n├¿bres') || c.spiritualite.nom.toLowerCase().includes('tenebres'));
    if (isTenebres) {
        const hp = c.hpCurrent !== undefined ? c.hpCurrent : c.healthCurrent;
        const maxHp = c.hpMax !== undefined ? c.hpMax : c.healthMax;
        ...
        const text = canCast ? 'Assombri' : 'Bloqu├®';
        const title = "Condition T├®n├¿bres : <= 80% PV ou Mana";
        statsHtml += `<span class="hero-stat-chip" title="${title}" style="border-color: ${borderColor}; color: ${color};"><span class="material-symbols-outlined" style="color: inherit;">${icon}</span>${text}</span>`;
    }
    ...
    if (isHero && c.specialItems) {
        ...
        if (specialItemKeys.length > 0) {
            ...
            specialItemKeys.forEach(k => {
                specialItemsHtml += `<span class="hero-stat-chip" title="Item Sp├®cial : ${k}" style="border-color: rgba(217, 70, 239, 0.4); color: #d946ef;"><span class="material-symbols-outlined" style="color: inherit; font-size: 1rem;">star</span>${c.specialItems[k]}x ${k}</span>`;
            });
    ...
    if (!isHero) {
        ...
        if (c.monsterType && c.monsterType !== 'NORMAL') {
            const typeTitles = {
                'DEMON': 'D├®mon : 10% des d├®g├óts inflig├®s le sont en d├®g├óts bruts suppl├®mentaires.',
                'REPTILE': 'Reptile : R├®duit les d├®g├óts physiques subis de 15%.',
                'MORT_VIVANT': 'Mort-vivant : R├®g├®n├¿re 5% de ses PV max au d├®but de son tour.',
                'HYBRIDE': 'Hybride : Utilise la plus haute valeur entre Force et Puissance pour attaquer.',
                'VAMPIRE': 'Vampire : Se soigne de 20% des d├®g├óts inflig├®s.'
            };
            ...
            const tLabel = { 'DEMON': 'D├®mon', 'REPTILE': 'Reptile', 'MORT_VIVANT': 'Mort-vivant', 'HYBRIDE': 'Hybride', 'VAMPIRE': 'Vampire' }[c.monsterType] || c.monsterType;
        ...
        if (c.behavior && c.behavior !== 'NORMAL') {
            const behaviorTitles = {
                'PREDATEUR': "Pr├®dateur : Verrouille une cible et l'attaque jusqu'├á sa mort.",
                'CORRUPTEUR': "Corrupteur : Cible toujours le joueur avec le plus de Mana et lui retire 5% Mana Act.",
                'LEADER': "Leader : Ordonne ├á tous les autres monstres d'attaquer sa cible.",
                'ASSASSIN': "Assassin : Vise syst├®matiquement le joueur avec le moins de R├®sistance.",
                'INSENSIBLE': "Insensible : Ses attaques infligent des d├®g├óts bruts (ignore l'armure)."
            };
            ...
            const bLabel = { 'PREDATEUR': 'Pr├®dateur', 'CORRUPTEUR': 'Corrupteur', 'LEADER': 'Leader', 'ASSASSIN': 'Assassin', 'INSENSIBLE': 'Insensible' }[c.behavior] || c.behavior;
    ...
    return `
        <div class="fighter-name" style="color: ${isHero ? '#f8fafc' : '#ef4444'}; font-size: 1.2rem; display: flex; justify-content: center; align-items: center; gap: 0.5rem; margin-bottom: 0.5rem;">
            ${isHero ? '­ƒºÖÔÇìÔÖé´©Å' : '­ƒæ╣'} ${titleIconsHtml} ${c.name}
        </div>
        ${monsterBadgesHtml}
        ${statsHtml}
        <div class="gauge-container" style="text-align: left;">
            <div class="gauge-label"><span>Sant├® (PV)</span><span>${hpLabel}</span></div>
    ...
    shieldList.forEach(s => {
        ...
        const entryHtml = `
            <div style="display:flex; align-items:flex-start; gap:0.4rem; font-size:0.85rem;">
                ...
                <span style="color:#e2e8f0;">Ô×ö ${s.amount} PV absorpt. (${s.duration} tours)</span>
            </div>
        `;
...
function renderBuffsHtml(buffList) {
    ...
    buffList.forEach(b => {
        ...
        const typeStr = (b.statAffected === 'POISON' || b.statAffected === 'BURN') ? ui.formatStat(b.statAffected) : 'Buff/D├®buff';
        ...
        const entryHtml = `
            <div style="display:flex; align-items:flex-start; gap:0.4rem; font-size:0.85rem;">
                ...
                <span style="color:#e2e8f0;">Ô×ö ${text} (${b.duration} tours)</span>
            </div>
        `;
    ...
    if (badBuffs.length > 0) {
        html += `<div class="sandbox-status-badge debuff" ${tooltipAttrs} style="cursor: help; position: relative;">
            ...
            <span>D├®buffs (${badBuffs.length})</span>
            <template class="tooltip-data">
                <div style="display: flex; flex-direction: column; gap: 0.3rem;">
                    ${badBuffs.join('')}
                </div>
...
function renderSpells(spells) {
    ...
    if (filteredSpells.length === 0) {
        container.innerHTML = '<div style="color: #94a3b8; font-style: italic; padding: 2rem; text-align: center;">Aucun sort dans cette cat├®gorie.</div>';
        return;
    }
    ...
    filteredSpells.forEach(sp => {
        ...
        if (currentSpellsTab === 'VOIE') {
            ...
            if (sp.spiritualite && sp.spiritualite.nom) {
                groupKey = sp.spiritualite.nom.toLowerCase().startsWith('spiritualit├®') || sp.spiritualite.nom.toLowerCase().startsWith('spiritualite')
                    ? sp.spiritualite.nom
                    : `Spiritualit├® de ${sp.spiritualite.nom}`;
            } else {
                groupKey = 'Autres';
            }
        } else {
...
function renderSpellCard(sp) {
    ...
    if (sp.castingType === 'INSTANTANE') {
        castingTypeHtml = '<span class="material-symbols-outlined" style="font-size: 1rem; color: #f59e0b;" title="Action Instantan├®e">bolt</span>';
    } else if (sp.castingType === 'CANALISE') {
        castingTypeHtml = '<span class="material-symbols-outlined" style="font-size: 1rem; color: #8b5cf6;" title="Action Canalis├®e">cyclone</span>';
        castingTypeHtml += sp.allowInstantDuringChanneling ?
            '<span class="material-symbols-outlined" style="font-size: 1rem; color: #f59e0b;" title="Instantan├®s autoris├®s pendant la canalisation">bolt</span>' :
            '<span style="position: relative; display: inline-flex; align-items: center; justify-content: center; width: 1rem; height: 1rem;" title="Instantan├®s interdits pendant la canalisation"><span class="material-symbols-outlined" style="font-size: 1rem; color: #64748b;">bolt</span><span style="position: absolute; width: 100%; height: 2px; background: #ef4444; transform: rotate(-45deg);"></span></span>';
    } else {
    ...
    if (sp.karmaAlignment === 'OFFENSIVE') {
        karmaAlignHtml = '<span class="material-symbols-outlined" style="font-size: 1rem; color: #c084fc;" title="Sort des T├®n├¿bres (Offensif)">dark_mode</span>';
    } else if (sp.karmaAlignment === 'PROTECTIVE') {
        karmaAlignHtml = '<span class="material-symbols-outlined" style="font-size: 1rem; color: #fde047;" title="Sort de Lumi├¿re (Protecteur)">light_mode</span>';
    ...
    let heatGenHtml = '';
    if (generatesHeat) {
        heatGenHtml = `<span class="material-symbols-outlined" style="font-size: 1rem; color: #f97316;" title="Sort G├®n├®rateur de Chaleur">local_fire_department</span>`;
    }
...
function showResult(playerWon) {
    ...
    if (playerWon) {
        ...
        desc.textContent = "Le donjon a ├®t├® compl├®t├®.";
    } else {
        title.textContent = "D├ëFAITE";
        title.style.color = "#ef4444";
        desc.textContent = "Votre ├®quipe a ├®t├® an├®antie.";
    }
